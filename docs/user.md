...
User will also need to install teflo . For teflo installation visit [here](https://teflo.readthedocs.io/en/latest/users/install.html#install-teflo)
...
## Credentials
In teflo.cfg, users will need to provide following information under the credential section:
...
gchat_url/slack_url are required when sending notifications to gchat/slack
...
Within teflo scenario descriptor file (SDF) following attributes can be set for
...
<table class="tg">
  ...
  <tr>
    ...
    <td class="tg-8m83">name for the notification block<br></td>
  ...
  <tr>
    ...
    <td class="tg-14gg">True (Value is notifier_service)</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td class="tg-14gg">Body of the message to be sent</td>
  ...
  <tr>
    ...
    <td class="tg-14gg">A dictionary of parameters used by the notify service rest multi api</td>
  ...
  <tr>
    ...
    <td class="tg-14gg">gchat remote template raw url</td>
    <td class="tg-14gg">String</td>
  ...
  <tr>
    ...
    <td class="tg-14gg">slack remote template raw url</td>
    <td class="tg-14gg">String</td>
    <td class="tg-14gg">False</td>
    <td class="tg-14gg"> This needs to be the raw url</td>
  </tr>
  ...
  <tr>
    ...
    <td class="tg-14gg">email remote template raw url</td>
    <td class="tg-14gg">String</td>
    <td class="tg-14gg">False</td>
    <td class="tg-14gg"> This needs to be the raw url</td>
  </tr>
  ...
  <tr>
    ...
    <td class="tg-14gg">A list of email addresses</td>
  ...
  <tr>
    ...
    <td class="tg-14gg">Topic to be sent on teh UBM</td>
    <td class="tg-14gg">String</td>
  ...
  <tr>
    ...
    <td class="tg-14gg">Message body for the message_bus</td>
    <td class="tg-14gg">Dict</td>
    <td class="tg-14gg">False</td>
    <td class="tg-14gg"> If provided a separate api call is made to send notification to the UMB. If not provided a single api call is made with all the targets included</td>
  </tr>
  </table>
   
  **Note**
  Following are some notes to keep in mind before putting together the notifictaion block for notify_service
  1. The template_url for gchat/slack/email need to be in raw format. One can get the raw url of template 
     stored on github/gitlab if you open up the template as **openraw on gitlab** and **raw on github** and 
     then copy the url from the browser.
  2. template url has higher precedence than the template_name
  3. When providing template_name , you need to make sure the template exist on the notify_service, else 
     it will result in an error. Please visit [here](https://github.com/waynesun09/notify-service#templates)
     to get more information on how to set up templates
  4. When sending notification to message_bus, if a message_request_body is provided in the SDF, a separate
     api call is made, else notifications are sent to all targets including message_bus in a single api call.
     This is because notify_service cannot verify different combinations the request body can have
...
To send notification to slack,gchat,email,irc,message_bus without the message_bus_request_body on completion
of the teflo execute task
...
5. **message bus** with topic "/topic/VirtualTopic.qe.ci.qedevops_teflo.brew-build.test.complete" and no 
   request body
...
To send notification to slack,gchat,email,irc, message_bus with the message_bus_request_body on failure of 
the teflo report task
...
To send notification on demand, run the notify command post the teflo run has completed providing the
results.yml as the scenario file or can run it anytime using the original SDF
